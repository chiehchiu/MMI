...
class BaseClassifier(BaseModule, metaclass=ABCMeta):
    ...
    def forward_test(self, imgs,pdesc,bics,bts, **kwargs):
        """
        Args:
        ...
        if len(imgs) == 1:
            return self.simple_test(imgs[0], pdesc,bics,bts,**kwargs)
        else:
            raise NotImplementedError('aug_test has not been implemented')

    @auto_fp16(apply_to=('img',))
    def forward(self, img, pdesc,bics,bts, return_loss=True, **kwargs):
        ...
        """
        if return_loss:
            return self.forward_train(img,pdesc,bics,bts, **kwargs)
        else:
            return self.forward_test(img,pdesc,bics,bts, **kwargs)
