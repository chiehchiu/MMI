...
class CAM():
    ...
    def getDLfeature(self, size = None):
        #print(self.values.activations.shape)
        #print(self.weight_fc.shape)
        return self.values.activations
    ...
    def getCAM(self, size=None):
        ...
        if len(self.values.activations.shape) == 5:
            cam = F.conv3d(self.values.activations.float(), weight=self.weight_fc[:, :, None, None, None])
            if size:
                cam = F.interpolate(cam, size=size, mode="trilinear", align_corners=False)
        else:
            cam = F.conv2d(self.values.activations.float(), weight=self.weight_fc[:, :, None, None])
            if size:
