...
from mmcv.runner import (BaseModule, ModuleList, Sequential, _load_checkpoint,
                         load_state_dict)
from ..utils import to_3tuple, to_2tuple
from mmcls.utils import get_root_logger
from ..builder import BACKBONES
from ..utils import PatchEmbed3D, ncdhw_to_nlc, nlc_to_ncdhw, pvt_convert
...
class MixFFN(BaseModule):
    ...
    def forward(self, x, dhw_shape, identity=None):
        out = nlc_to_ncdhw(x, dhw_shape)
        out = self.layers(out)
        out = ncdhw_to_nlc(out)
        if identity is None:
            identity = x
...
class SpatialReductionAttention(MultiheadAttention):
    ...
    def __init__(self,
        ...
        if sr_ratio > 1:
            if fixz_sr_ratio is not None:
                sr_kernel_size = (fixz_sr_ratio, sr_ratio, sr_ratio)
            else:
                sr_kernel_size = sr_ratio
            self.sr = Conv3d(
                in_channels=embed_dims,
                out_channels=embed_dims,
                kernel_size=sr_kernel_size,
                #stride=sr_ratio) # incorrect impl
                stride=sr_kernel_size)
    ...
    def forward(self, x, dhw_shape, identity=None):
        ...
        x_q = x
        if self.sr_ratio > 1:
            x_kv = nlc_to_ncdhw(x, dhw_shape)
            x_kv = self.sr(x_kv)
            x_kv = ncdhw_to_nlc(x_kv)
            x_kv = self.norm(x_kv)
        else:
            x_kv = x
    ...
    def legacy_forward(self, x, dhw_shape, identity=None):
...
class PVTEncoderLayer(BaseModule):
    ...
    def forward(self, x, dhw_shape):
        x = self.attn(self.norm1(x), dhw_shape, identity=x)
        x = self.ffn(self.norm2(x), dhw_shape, identity=x)
...
class AbsolutePositionEmbedding(BaseModule):
    ...
    def resize_pos_embed(self, pos_embed, input_shape, mode='trilinear'):
        ...
        Resize pos_embed using trilinear interpolate method.
        ...
        pos_d, pos_h, pos_w = self.pos_shape
        pos_embed_weight = pos_embed[:, (-1 * pos_h * pos_w * pos_d):]
        pos_embed_weight = pos_embed_weight.reshape(
            1, pos_d, pos_h, pos_w, self.pos_dim).permute(0, 4, 1, 2, 3).contiguous()
    ...
    def forward(self, x, dhw_shape, mode='trilinear'):
        pos_embed = self.resize_pos_embed(self.pos_embed, dhw_shape, mode)
        return self.drop(x + pos_embed)


@BACKBONES.register_module()
class PyramidVisionTransformer3D(BaseModule):
    """Pyramid Vision Transformer3D (PVT3D)

    3D Implementation of `Pyramid Vision Transformer: A Versatile Backbone for
    ...
    def __init__(self,
                 ...
                 num_layers=[3, 4, 6, 3],
                 num_heads=[1, 2, 5, 8],
                 patch_sizes=[(4,4,4), 2, 2, 2],
                 strides=[(1,4,4), (1,2,2), (1,2,2), (1,2,2)], # strides[0] must be 3-element tuple
                 paddings=[(0,0,0), 0, 0, 0],
                 sr_ratios=[8, 4, 2, 1],
                 fixz_sr_ratio=[1, 1, 1, 1], # Set to None to use sr_ratios for depth-dims
                 out_indices=(0, 1, 2, 3),
                 mlp_ratios=[8, 8, 4, 4],
                 qkv_bias=True,
                 drop_rate=0.,
        ...
        for idx, patch_size in enumerate(patch_sizes):
            if isinstance(patch_size, int):
                patch_sizes[idx] = to_3tuple(patch_size)
        self.patch_sizes = patch_sizes
        for idx, stride in enumerate(strides):
            if isinstance(stride, int):
                strides[idx] = to_3tuple(stride)
        self.strides = strides
        for idx, padding in enumerate(paddings):
            if isinstance(padding, int):
                paddings[idx] = to_3tuple(padding)
    ...
    def forward(self, x):
        ...
        for i, layer in enumerate(self.layers):
            x, dhw_shape = layer[0](x)

            for block in layer[1]:
                x = block(x, dhw_shape)
            x = layer[2](x)
            x = nlc_to_ncdhw(x, dhw_shape)
            if i in self.out_indices:
                if self.output_2d:
                    B, C, D, H, W = x.shape
                    outs.append(x[:,:,D//2,:,:].view(B,C,H,W))
                else:
                    outs.append(x)
