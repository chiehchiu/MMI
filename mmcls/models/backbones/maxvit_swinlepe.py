...
class ShiftWindowMSALePE(BaseModule):
    ...
    def forward(self, query, hw_shape):
        ...
        if self.use_grid_attn:
            # B H' W' C
            shifted_x = self.grid_reverse(attn_windows, H_pad, W_pad)
        else:
            # B H' W' C
    ...
    def grid_reverse(self, grids, H, W):
        """
        Args:
            grids: (num_grids*B, grid_size, grid_size, C)
            H (int): Height of image
            W (int): Width of image
        Returns:
            x: (B, H, W, C)
        ...
        x = grids.view(B, H // grid_size, W // grid_size, grid_size, 
                       grid_size, -1)
    ...
    def __init__(self,
        ...
        for i in range(depth):
            if i in target_layer:
                ...
                if use_global_shift:
                    raise NotImplementedError("Deprecated options")
                block = SwinLePEBlock(
                    embed_dims=embed_dims,
                    num_heads=num_heads,
                    feedforward_channels=feedforward_channels,
...
class MaxvitSwinLePETransformer(BaseModule):
    ...
    def __init__(self,
        ...
        super(MaxvitSwinLePETransformer, self).__init__(init_cfg=init_cfg)
        ...
        for i in range(num_layers):
            ...
            if mixed_arch and pvt_stage[i]: # add pvt block to SwinLePE 
                ...
                #assert depths[i] % 3 == 0, "depth must be divisible by three for win/shift-win/axis-win arch"
    ...
    def train(self, mode=True):
        ...
        super(MaxvitSwinLePETransformer, self).train(mode)
        self._freeze_stages()
