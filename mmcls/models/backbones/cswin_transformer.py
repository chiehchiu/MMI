...
class LePEAttention(BaseModule):
    def __init__(self, dim, resolution, idx, split_size=7, dim_out=None, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., fix_grid_size=False, init_cfg=None):
        """Not supported now, since we have cls_tokens now.....
        """
        super().__init__(init_cfg)
        self.dim = dim
        self.dim_out = dim_out or dim
        self.fix_grid_size = fix_grid_size
...
class Merge_Block(BaseModule):
    def __init__(self, dim, dim_out, norm_layer=nn.LayerNorm, init_cfg=None):
        super().__init__(init_cfg)
...
class CSWin(BaseModule):
    ...
    """
    def __init__(self, img_size=224, patch_size=4, in_chans=3, embed_dim=64, depth=[1,2,21,1], split_size = 7, out_indices=(3,),
                 num_heads=[1,2,4,8], mlp_ratio=4., qkv_bias=False, qk_scale=None, drop_rate=0., attn_drop_rate=0.,
                 drop_path_rate=0., fix_grid_size=False, hybrid_backbone=None, norm_layer=nn.LayerNorm, use_chk=False, init_cfg=None):
        ...
        self.merge1 = Merge_Block(curr_dim, curr_dim*(heads[1]//heads[0]), init_cfg=None)
        ...
        self.merge2 = Merge_Block(curr_dim, curr_dim*(heads[2]//heads[1]), init_cfg=None)
        ...
        self.merge3 = Merge_Block(curr_dim, curr_dim*(heads[3]//heads[2]), init_cfg=None)
