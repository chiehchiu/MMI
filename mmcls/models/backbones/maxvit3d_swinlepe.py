...
class ShiftWindowMSA3DLePE(BaseModule):
    ...
    def __init__(self,
        ...
        self.use_grid_attn = use_grid_attn
    ...
    def forward(self, query, dhw_shape):
        ...
        if self.shift_size[0] > 0:
            assert not self.use_grid_attn, "shift size must be 0 when self.use_grid_attn=True" 
        ...
        if self.use_grid_attn:
            # nW*B, window_size, window_size, window_size, C
            query_windows = self.grid_partition(shifted_query)
        else:
            # nW*B, window_size, window_size, window_size, C
        ...
        if self.use_grid_attn: 
            # B D' H' W' C
            shifted_x = self.grid_reverse(attn_windows, D_pad, H_pad, W_pad)
        else:
            # B D' H' W' C
    ...
    def grid_reverse(self, grids, D, H, W):
        """
        Args:
            windows: (num_grids*B, grid_size, grid_size, grid_size, C)
            D (int): Depth of image
            H (int): Height of image
            W (int): Width of image
        Returns:
        ...
        B = int(grids.shape[0] / (H * W * D/ grid_size[0] / grid_size[1] / grid_size[2]))
        x = grids.view(B, D // grid_size[0], H // grid_size[1], W // grid_size[2],
    ...
    def grid_partition(self, x):
        ...
        Args:
            x: (B, D, H, W, C)
        Returns:
            windows: (num_grids*B, grid_size[0], grid_size[1], grid_size[2], C)
        """
        B, D, H, W, C = x.shape
        grid_size = self.grid_size
        x = x.view(B, grid_size[0], D // grid_size[0],
                      grid_size[1], H // grid_size[1],
                      grid_size[2], W // grid_size[2], C)
        grids = x.permute(0, 2, 4, 6, 1, 3, 5, 7).contiguous()
        grids = grids.view(-1, grid_size[0], grid_size[1], grid_size[2], C)
        return grids
...
class SwinLePEBlock(BaseModule):
    ...
    def __init__(self,
        ...
        if self.parallel_group:
                        assert not use_grid_attn, "use grid attn must be Fasle when parallel_group is True"
    ...
    def __init__(self,
        ...
        for i in range(depth):
            block = SwinLePEBlock(
                ...
                feedforward_channels=feedforward_channels,
                window_size=window_size,
                shift=True if i % 3 == 1 else False, # use shift attn after win attn
                use_grid_attn=True if i % 3 == 2 else False, # use grid atten after win&shift-win attn
...
class MaxvitSwinLePETransformer3D(BaseModule):
    ...
    def __init__(self,
        ...
        super(MaxvitSwinLePETransformer3D, self).__init__(init_cfg=init_cfg)
        ...
        assert strides[0][2] == patch_size[2], 'Use non-overlapping patch embed.'  
    ...
    def train(self, mode=True):
        ...
        super(MaxvitSwinLePETransformer3D, self).train(mode)
        self._freeze_stages()
