...
class MergeChannel(object):
    """merge multiple input tensors in the 0 dim to form multiple-channel input.

    Args:
        key (list of str): saved keys to be merged
    """

    def __init__(self, keys=['img', 'blood']):
        self.keys = keys
    ...
    def __call__(self, results):
        ...
        for key in self.keys:
            img_list.append(np.expand_dims(results[key], 0))

        merged_img = np.concatenate(img_list, axis=0)
        results['img'] = merged_img

        return results

    def __repr__(self):
        return self.__class__.__name__ + f'(merged keys={self.keys})'
...
class PhotoMetricDistortionMultipleSlices(object):
    ...
    def __repr__(self):
        ...
        repr_str += ('(brightness_delta={}, contrast_range={})').format(
...
@PIPELINES.register_module()
class TensorNormCropFlip:
    def __init__(self, crop_size, move=5, train=True, mean=99., std=77., copy_channels=False, dim_type='hwd'):
        ...
        if dim_type == 'hwd':
            ...
            raise ValueError('unsupport input dimmension type')
    ...
    def __call__(self, results):
        ...
        if self.train:
            ...
            voxel_ret = self.crop(voxel, center, self.size)
...
def random_center(shape, move):
    offset = np.random.randint(-move, move + 1, size=len(shape))
    zyx = np.array(shape) // 2 + offset
    return zyx

def crop_hwd(array, center, size):
...
def crop_cdhw(array, center, size):
    # For input tensor (c, d, h, w), crop along xy axis
    _, _, y, x = center
    h, w, _ = size
    cropped = array[...,y - h // 2:y + h // 2,
                    x - w // 2:x + w // 2]
...
class Resize3DTensor(object):
    ...
    def __init__(self,
        ...
        assert interpolation in ('nearest', 'trilinear')
    ...
    def _resize_img(self, results):
        for key in results.get('img_fields', ['img']):
            ...
            if len(img_shape) == 4:
                img_c = 1
                img_b, img_d, img_h, img_w = img_shape
                img = img.view(-1,1,img_d,img_h,img_w)
                new_shape = (img_b,) + self.size
            # b,c,d,h,w
            elif len(img_shape) == 5:
                img_b, img_c, img_d, img_h, img_w = img_shape
                img = img.view(-1,1,img_d,img_h,img_w)
                new_shape = (img_b, img_c) + self.size
            else:
                print("not support shape")
                raise
            resize_img = F.upsample(img,new_shape,mode=self.interpolation)
            resize_img = resize_img.view(new_shape)
            results[key] = resize_img
