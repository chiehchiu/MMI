...
def build_dataloader(dataset,
    ...
    assert sid_sampler in ['default', 'sid', 'patch3d', 'ct']
    if sampler_cfg:
    ...
    # Default sampler logic
        ...
        default_args = None
        if sid_sampler == 'default':
            sampler_type = 'DistributedSampler'
        elif sid_sampler == 'sid':
            sampler_type = 'DistributedSIDSampler'
        elif sid_sampler == 'patch3d':
            sampler_type = 'Distributed3DPatchSampler'
        elif sid_sampler == 'ct':
            sampler_type = 'DistributedCTSampler'
            default_args = {num_per_ct:num_per_ct, pos_fraction:pos_fraction}
        sampler = build_sampler(
            dict(
                type=sampler_type,
                dataset=dataset,
                ...
                rank=rank,
                shuffle=shuffle,
                round_up=round_up), default_args=default_args)
    else:
        sampler = None
