...
model = dict(
    ...
    head=dict(
        ...
        num_classes=2,
        in_channels=256,
        loss=dict(type='CrossEntropyLoss', use_sigmoid=False, loss_weight=1),
        multi_cls=True
    ))
# dataset settings
dataset_type = 'huaxiMultiPneCriticalDataset'
img_norm_cfg = dict(
...
root_dir = '/app_data/majiechao/CAMAS/pne_multi_data/dataset/'
data = dict(
    samples_per_gpu=32,
    workers_per_gpu=4,
    num_per_ct=32,
    ...
    train=dict(
        ...
        ann_file= root_dir + 'pne_2clscritical_train.csv',
        pipeline=train_pipeline),
    val=dict(
        type=dataset_type,
        data_prefix=root_dir + 'ct_data',
        ann_file= root_dir + 'pne_2clscritical_test.csv',
