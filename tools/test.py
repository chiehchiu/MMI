...
def main():
    ...
    if not distributed:
        ...
        if args.cam2nii or args.cam2jpg:
            outputs = single_gpu_test_cam3d(model,data_loader,cam2nii=args.cam2nii,cam2jpg=args.cam2jpg,is_2d=is_2d)
        else:
        ...
        if args.metrics:
            ...
            for k, v in eval_results.items():
                ...
                if isinstance(v, np.ndarray):
                    v = [round(out, 4) for out in v.tolist()]
                elif isinstance(v, Number):
                    v = round(v, 4)
                elif isinstance(v, dict):
                    pass
                elif isinstance(v, str): # add for extend_aneursym evaluation
                    pass
                else:
                    raise ValueError(f'Unsupport metric type: {type(v)}')
                print(f'{k} : {v}')
        if 'predict' in eval_results:
            predict_list = eval_results['predict']
            # save
            save_file = '/app_data/daihesen/2.pne_cls/predict.txt'
